...
mainApp.controller('loginController', function ($scope, $http, $filter, $cookies) {
    $cookies.put('user-type', 'admin');
});

mainApp.controller('adminController', function ($scope, $http, $filter, $location, $cookies, $sce, $interval) {
	...
	$scope.callLogger = function() {
		var urlLogger = "/api/logger/last/10";
		$http.get(urlLogger).success(function (response) {
